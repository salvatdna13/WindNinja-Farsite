...
def field_winds(wind):
    start_time_local = time.time()
    month, day, hour, speed, direction, cover = wind[1]
    try:        
        if args.output[-1] != '/':
            args.output += '/'
    ...
    command = ['WindNinja_cli', args.config_wnd, '--output_path', path, '--month', month, '--day', day, 
    ...
    try:
        with open(path + 'time_50m.txt', 'w') as time_file:
    ...
    print('(' + str(wind[0]) + '/' + str(total) + '): ' + str(end_time_local-start_time_local) + ' s', flush=True)
    return atms
...
if __name__ == '__main__':
    ...
    atm_filename = 'case_1_50m.atm'
    if args.windninja:
        try:
            with open(args.windfile, 'r') as windfile:
                winds = {}
                for iter, line in enumerate(windfile):
        ...
        with multiprocessing.Pool() as pool:
            chunked_winds = [[i, j] for i, j in zip(winds.keys(), winds.values())]
            atms = pool.map(field_winds, chunked_winds)
        ...
        try:
            with open(atm_filename, 'w') as atm_file:
                atm_file.writelines('WINDS_AND_CLOUDS\n')
                for atm in atms:
                    atm_file.writelines(atm)
        except IOError:
